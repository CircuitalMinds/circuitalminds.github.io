...
class HeadTemplates:
    ...
    def __init__(self):
        ...
        for song in list(self.music_meta_tags.keys()):            
            self.save_template(template=self.build_music_template(song_data=self.music_meta_tags[song]), name=f'./music_app/{song}')
        for post in self.blog_posts:            
            self.save_template(template=self.build_blog_template(post=post, section="blog"), name=f'./blog/{post}')
        for post in self.pyfullstack_posts:            
            self.save_template(template=self.build_blog_template(post=post, section="pyfullstack"), name=f'./pyfullstack/{post}')
